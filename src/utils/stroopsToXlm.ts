...
function normalizeToBigInt(input: number | string | bigint): bigint {
  ...
  if (typeof input === "number") {
    if (!Number.isInteger(input)) {
      throw new TypeError(
        `stroopsToXlm: expected an integer stroop amount, received non-integer number ${input}. ` +
          `Pass a string or BigInt for large/precise values instead of a float.`
      );
    }
    if (!Number.isSafeInteger(input)) {
      throw new TypeError(
        `stroopsToXlm: ${input} exceeds Number.MAX_SAFE_INTEGER and may already have lost precision. ` +
  ...
  if (!/^-?\d+$/.test(trimmed)) {
    throw new TypeError(
      `stroopsToXlm: "${input}" is not a valid integer stroop amount string.`
    );
  }
