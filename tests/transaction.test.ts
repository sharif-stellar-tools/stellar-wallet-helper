import { TxManager } from '../src/transaction';
...
describe('TxManager', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    txManager = new TxManager('https://horizon-testnet.stellar.org');
  });

  it('successfully builds a payment transaction with dynamic fee and dynamic network passphrase', async () => {
    const sourceKeypair = WalletManager.createWallet();
    const destKeypair = WalletManager.createWallet();
    ...
    const mockAccount = new Account(sourceAddress, '100');
